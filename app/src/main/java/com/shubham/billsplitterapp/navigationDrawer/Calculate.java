            ...
            for(int j=0;j<Doner.size();j++){
                ...
                for(int k=0;k<Receiver.size();k++){
                    ...
                    if(Integer.parseInt(Doner.get(j).get(1))>=Integer.parseInt(Receiver.get(k).get(1))){
                        ...
                        payeeName = Receiver.get(k).get(0).substring(2);
                        Doner.get(j).set(1,"0");
...
//                    Toast on Payment..
                    if(payList.get(j).get(m-3) != "Don't have to pay anything"){
                        int finalJ = j;
                        ((TextView)item.getSubItemView(m).findViewById(R.id.child_item)).setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View view) {
                                TastyToast.makeText(Calculate.this, "Pay Rs. "+ dataList.get(finalJ).get(2) +" to "+ payeeName + " Via UPI..", TastyToast.LENGTH_SHORT, TastyToast.SUCCESS);

                                EasyUpiPayment.Builder builder = new EasyUpiPayment.Builder(Calculate.this)
                                        .setPayeeVpa(payeeName+"@paytm")
                                        .setPayeeName(dataList.get(finalJ).get(0).substring(2))
                                        .setPayeeMerchantCode("12345")
                                        .setTransactionId("T2020090212345")
                                        .setTransactionRefId("T2020090212345")
                                        .setDescription("Pay Divided Amount to "+payeeName)
                                        .setAmount("101.88");
